...
// Generated inputs are data, never executable modules or descriptor-selected paths.
async function readFixedGeneratedFile(root, name, maxBytes) {
  let current = await realpath(root);
  for (const component of ['src', 'generated', 'scheduled-release', name]) {
    current = path.join(current, component);
    const stat = await lstat(current);
    if (stat.isSymbolicLink() || (component === name ? !stat.isFile() : !stat.isDirectory())) {
      throw new Error('Unsupported generated input layout.');
    }
  }
  ...
  try {
    ...
    if (size > maxBytes) throw new Error('Generated input exceeds limit.');
...
async function readPublishedGeneratedArticle(root, now) {
  try {
    const descriptor = (await readFixedGeneratedFile(root, 'server.mjs', 256 * 1024)).toString('utf8');
    const prefix = '// Generated by scripts/stage-scheduled-release.mjs. Do not edit.\nconst release = Object.freeze(';
    const suffix = ');\n\nexport default release;\n';
    if (!descriptor.startsWith(prefix) || !descriptor.endsWith(suffix)) return null;
    const release = JSON.parse(descriptor.slice(prefix.length, -suffix.length));
    if (!release || release.schemaVersion !== 1
      || typeof release.editionId !== 'string' || !/^edition-[a-z0-9-]+$/.test(release.editionId)
      || typeof release.publishAt !== 'string'
      || !/^\d{4}-\d{2}-\d{2}T(?:[01]\d|2[0-3]):[0-5]\d:[0-5]\d(?:\.\d{3})?(?:Z|[+-](?:[01]\d|2[0-3]):[0-5]\d)$/.test(release.publishAt)
      || !validCalendarDate(release.publishAt.slice(0, 10))
      || !Number.isFinite(release.publishAtMs) || Date.parse(release.publishAt) !== release.publishAtMs
      || release.article?.file !== 'article.md' || typeof release.article.sha256 !== 'string'
      || !/^[a-f0-9]{64}$/.test(release.article.sha256)
      || now.getTime() < release.publishAtMs) return null;

    // Decide eligibility before opening article bytes, including on malformed clocks.
    const bytes = await readFixedGeneratedFile(root, 'article.md', 1024 * 1024);
    if (createHash('sha256').update(bytes).digest('hex') !== release.article.sha256) return null;
    ...
    if (data.id !== release.editionId || data.publishAt !== release.publishAt || data.status !== 'scheduled'
      ...
      || typeof data.summary !== 'string' || !data.summary.trim()) return null;
    const url = `https://longmontai.com/edition/${data.id}`;
    return {
      id: data.id, date: data.date, title: data.title, summary: data.summary,
      path: url,
      sourceUrls: [url, ...sourceUrlsFromMarkdown(body)],
      headings: Array.from(body.matchAll(/^#{2,3}\s+(.+)$/gm)).map((match) => match[1]).slice(0, 8),
    };
  } catch {
    // Missing, malformed, or unsupported generated inputs never expose private data.
    return null;
  }
}
...
async function readRecentArticles(root, now, limit = 6) {
  ...
  const generated = await readPublishedGeneratedArticle(root, now);
  // An eligible static promotion is authoritative; future static entries are not.
  if (generated && !articles.some((article) => article.id === generated.id)) articles.push(generated);
  return articles
...
export async function collectWebsiteSignals({
  ...
  if (!(now instanceof Date) || !Number.isFinite(now.getTime())) throw new Error('Invalid newsletter collection clock.');
