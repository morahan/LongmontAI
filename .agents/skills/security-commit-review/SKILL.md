...
- Agent control-plane files under `.github/`, `.codex/`, `.agents/`, `.githooks/`, `api/`, and `scripts/`, plus `package.json` and `vercel.json`, for prohibited full-access sandboxes, broad workflow writes, and persisted checkout credentials. Push/manual scans read immutable archived snapshots, not the worktree.
